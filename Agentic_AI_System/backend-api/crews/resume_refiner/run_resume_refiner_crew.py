...
def run_upload(file):
    """Save uploaded file (PDF, JPG, PNG) and return upload_id"""
    crew = ResumeRefinerCrew()
    return crew.save_upload(file)

def run_parse(upload_id):
    """Parse document (PDF or image) and extract sections"""
    crew = ResumeRefinerCrew()
    return crew.parse_document(upload_id)

def run_analyze_layout(upload_id):
    """Analyze document layout (PDF or image)"""
    crew = ResumeRefinerCrew()
    return crew.analyze_document_layout(upload_id)
...
def run_evaluate(upload_id):
    ...
    resume_data = crew.parse_document(upload_id)
    
    # Get layout data
    layout_data = crew.analyze_document_layout(upload_id)
